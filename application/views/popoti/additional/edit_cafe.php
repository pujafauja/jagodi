<form action="<?php echo base_url('Cafe/cafe_data/'.encode($id)) ?>" data-type="edit" method="POST">
				...
				<div class="row">
					...
					<div class="col-md-6">
						<div class="input-group mb-3">
							<div class="input-group-prepend">
								<a class="btn btn-primary popup-item" href="<?php echo base_url('cafe/foods') ?>"><i class="fas fa-search"></i></a>
							</div>
				...
				<table class="table table-striped table-ordered table-sm" id="tbl-transaction">
					...
					<tbody>
						<?php foreach (json_decode($row->detail) as $key => $value): ?>
							<tr data-id="<?php echo $value->cafeid ?>">
	                            <td>
	                            	<a class="delete-item text-danger mr-1"><i class="fas fa-times"></i></a><?php echo sql_get_var('tb_cafe_prices', 'nama', ['id' => $value->cafeid ]) ?>
	                            	<input type="hidden" name="kode-barang[]" value="<?php echo $value->cafeid ?>">
	                            </td>
	                            <td>
