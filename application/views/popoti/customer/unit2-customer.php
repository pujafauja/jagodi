    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <table id="unit-datatable" class="table dt-responsive nowrap w-100">
                    <thead>
                        <tr>
                            <th>Jenis</th>
                            <th>Merk</th>
    ...
    ( function ( $ ) {
        ...
        if( $.isFunction($.fn.DataTable) )
        {
            var dataTable = $('#unit-datatable').DataTable( {
                ...
                "ajax":{
                    url : base_url+"customer/unit-json?popup=1",
                    type: "post",
                    error: function(){ 
        ...
        $(document).on('click', '.chooseUnit', function(e){
            var jenis    = $(this).closest('tr').find('td').eq(0).text();
            var merk     = $(this).closest('tr').find('td').eq(1).text();
            var unit     = $(this).closest('tr').find('td').eq(2).text();
            var kategori = $(this).closest('tr').find('td').eq(3).text();

            $('input[name="unit-id"]').val($(this).data('id'));
            $('input[name="unit"]').val(unit+' '+kategori+' '+merk);
            $('input[name="merk"]').val(merk);
            $('input[name="jenis"]').val(jenis);
            $('input[name="kategori"]').val(kategori);
            $('input[name="category_id"]').val($(this).data('category'));
