...
/**
 * Kelompok_barang Model
 */
class Kelompok_barang_model extends CI_Model
{
	...
	function get($id = 0, $filter = array(), $hlm = 1, $totrows = false, $order_by = false, $limit = false)
	{
		...
		$roots = $this->db->get('tb_kelompok')->result_array();
	...
	function saving($datas) {
		$this->db->insert('tb_kelompok', $datas);
	}

	function update($datas, $id) {
		$this->db->where('id', $id);
		$this->db->update('tb_kelompok', $datas);
	}
