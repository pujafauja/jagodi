...
class Retail_model extends CI_Model
{
	...
	{
		$this->db->insert('tb_retail_prices', $data);
	}

	function updateprices($data, $id)
	{
		$this->db->where('id', $id);
		$this->db->update('tb_retail_prices', $data);
	}

	function rowprices($where)
	{
		$this->db->where($where);
		return $this->db->get('tb_retail_prices');
	}

			function getretail($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		$sql = "
			SELECT 
				a.`id`,
