 ...
 * Kode_bantu Model
 */
class Kode_bantu_model extends CI_Model
{
	...
	function get($id = 0, $filter = array(), $hlm = 1, $totrows = false, $order_by = false, $limit = false)
	{
		...
		$roots = $this->db->get('tb_kode_bantu')->result_array();
	...
	function saving($datas) {
		$this->db->insert('tb_kode_bantu', $datas);
	}

	function update($datas, $id) {
		$this->db->where('id', $id);
		$this->db->update('tb_kode_bantu', $datas);
	}

	function delete($id) {
		$this->db->where('id', $id);
		$this->db->delete('tb_kode_bantu');
	}
}
?>