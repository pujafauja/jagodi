( function ( $ ) {
    ...
    if( $.isFunction($.fn.DataTable) )
    {
        var dataTable = $('#prices-datatable').DataTable( {
            ...
            "oLanguage": {
                ...
                "sLengthMenu": "_MENU_ &nbsp;&nbsp;per Page <a href='"+base_url+"retail/tambah-prices' class='btn btn-danger waves-effect waves-light ml-3' id='TambahPrices'><i class='mdi mdi-plus-circle mr-1'></i> Add New</a><span id='Notifikasi' style='display: none;'></span>",
            ...
            "ajax":{
                url : base_url+"retail/prices-json",
                type: "post",
                error: function(){ 
    ...
    $('#find-item').autocomplete({
        ...
        serviceUrl: base_url + 'retail/data-json',
            ...
            success: function(data){
                if(data.status == 1)
                {
                    Swal.fire({
                        ...
                        {
                            window.open(
                                base_url + 'additional/print-wo/retail/' + data.retailid + '?cetak=1',
                                'popUpWindow',
                                'height=567,width=793,left=10,top=10,retailidesizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no,status=yes')
                            if (id == 'edit') {
